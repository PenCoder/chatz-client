...
function ChatRoom() {
    ...
    useEffect(() => {
        ...
        const con = new HubConnectionBuilder()
            .withUrl(`https://localhost:5001/hubs/chatter`)
        ...
        const onSignedIn = async () => {
            ...
            try {
                let response = await fetch('https://localhost:5001/api/users/signed_in', {
                    method: 'POST',
                    headers: {
    ...
    const onLogout = async () => {
        try {
            let response = await fetch(`https://localhost:5001/api/users/logout?email=${email}`);
    ...
    const onSubmitMsg = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            var response = await fetch(`https://localhost:5001/api/message/send?` + params);
    ...
    const getUserMessages = async (user: any) => {
        try {
            ...
            var response = await fetch(`https://localhost:5001/api/message/messages/?sender=${email}&receiver=${userEmail}`);
    ...
    const onBlockUser = async () => {
        ...
        let response = await fetch(`https://localhost:5001/api/users/block_user?user=${userEmail}&by=${email}`);
        let results = await response.text();
        if (results === 'success') {
    ...
    const onUnBlockUser = async () => {
        ...
        let userEmail = (selectedUser as any).email;
        let response = await fetch(`https://localhost:5001/api/users/unblock_user?user=${userEmail}&by=${email}`);
        let results = await response.text();
        if (results === 'success') {
            let user = selectedUser as any;
            user['status'] = 'active';
    ...
    const getUsers = async () => {
        ...
        try {
            let res = await fetch(`https://localhost:5001/api/users/load_users?email=${email}`);
